...
> この手順はアプリの**初期設定画面にも表示される**ので、READMEを見ながら作業する必要はありません。Redirect URI はコピーボタンで貼り付けられます。
...
2. 初回のみ、画面の手順に沿って **Client ID** を入力 → **Spotifyでログイン** で認可
3. **アルバム追加** タブ(PCでは右ペイン)で、追加したいアルバムを選択
   - アーティスト単位の折りたたみ表示。**アルバム数の多いアーティストが上**に並び、初期状態は全て閉じています。行をタップで開閉、**すべて開く / すべて閉じる** で一括切り替え
   - アルバム名・アーティスト名でインクリメンタル検索(検索中は該当グループが自動で開きます)
...
「保存済みアルバム」(`GET /me/albums`)に加えて、**「いいねした曲」(♡)の収録アルバム**も追加候補として表示します。アルバム自体をライブラリに保存していなくても、曲だけ♡していれば候補に出ます(カードに「♡曲より」と表示)。♡曲の走査は画面表示後にバックグラウンドで進み、完了すると一覧に反映されます。

見出しの「(N枚)」が候補の総数です。それでも見当たらないアルバムがある場合は、Spotifyアプリでそのアルバムを開いて♡(ライブラリに保存)してから、このページを再読み込みしてください。
...
| いいねした曲 | `GET /me/tracks` (limit=50, 全件ページング) | 未保存アルバムの取りこぼしを補う。スコープは `user-library-read` で共通 |
